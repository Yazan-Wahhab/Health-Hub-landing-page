...
// =========================================================================
const Falling3DShapes = React.memo(({ isMobile }: { isMobile: boolean }) => {
  ...
  return (
    <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1400px] h-full z-[-1] pointer-events-none hidden lg:block transform-gpu [-webkit-backface-visibility:hidden]">
      {shapesData.map((shape) => (
        <motion.div
          key={shape.id}
          className={`absolute top-[-60px] ${shape.size} will-change-transform transform-gpu`}
...
export default function Header() {
  ...
  const [navState, setNavState] = useState<"horizontal" | "ball" | "vertical">(
    "horizontal",
  );
  ...
  useMotionValueEvent(scrollY, "change", (latest) => {
    ...
    if (isScrolledPast && navStateRef.current === "horizontal") {
      ...
      !isScrolledPast &&
      (navStateRef.current === "vertical" || navStateRef.current === "ball")
    ) {
      navStateRef.current = "ball";
      setNavState("ball");
  ...
  const navVariants = {
    ...
    vertical: {
      ...
      maxWidth: isMobile ? 64 : 72,
      height: isMobile ? 64 : 600,
  ...
  if (!mounted)
    return <div className="fixed inset-0 z-[100] pointer-events-none" />;
      ...
      <AnimatePresence>
        {isMenuOpen && isMobile && (
          <motion.div
            ...
            className="fixed inset-0 z-[90] bg-slate-900/10 backdrop-blur-[2px] pointer-events-auto transform-gpu"
          />
        )}
      </AnimatePresence>
        ...
        <motion.nav
          ...
          className={`absolute pointer-events-auto flex items-center justify-between bg-[#cbf0df]/85 ring-1 ring-[#0d9468]/20 overflow-hidden transform-gpu [-webkit-backface-visibility:hidden] ${
            isMobile
              ...
              : "backdrop-blur-2xl shadow-[inset_0_1px_1px_rgba(255,255,255,0.8),0_12px_40px_rgba(13,148,104,0.15)]"
          }`}
          style={{
            ...
            padding: isMobile
              ...
              : isVertical
                ...
                : "0px 28px",
            justifyContent:
              isMobile && navState !== "horizontal"
                ? "center"
                : "space-between",
          }}
        >
          <Falling3DShapes isMobile={isMobile} />
          ...
          {!isMobile && (
            <>
              <a
                href="/#home"
                onClick={(e) => scrollToSection(e, "home")}
                className="flex items-center gap-3 group relative shrink-0 z-20 cursor-pointer"
              >
                <div
                  className={`relative flex items-center justify-center transition-all duration-500 group-hover:scale-110 group-hover:rotate-3 ${isVertical || isBall ? "w-12 h-12" : "w-10 h-10"}`}
                >
                  <img
                    src="https://newworkspace.health-hubs.net/_next/image?url=%2Fassets%2Fimages%2Ffacicon.png&w=1080&q=75"
                    alt="Health Hub"
                  ...
                  {navState === "horizontal" && (
                    ...
                    >
                      <h1 className="text-[20px] font-display font-extrabold tracking-tight text-[var(--color-text-main)] leading-none transition-colors duration-300 group-hover:text-[var(--color-primary)]">
                        Health-
                        <span className="text-[var(--color-primary)] group-hover:text-[var(--color-secondary)] transition-colors duration-300">
                          Hub
                        </span>
                      </h1>
                    </motion.div>
              ...
              <AnimatePresence>
                {!isBall && (
                  <motion.div
                    ...
                    className={`flex items-center relative z-10 ${isVertical ? "flex-col gap-2 mt-4 mb-4 w-full px-3" : "flex-row gap-1 lg:gap-1.5"}`}
                  >
                    {NAV_ITEMS.map((item) => {
                      const isActive =
                      ...
                      return (
                        <a
                          ...
                          className={`relative flex items-center justify-center group transition-all duration-300 z-20 cursor-pointer ${isVertical ? "w-full h-12 rounded-xl" : "px-3 lg:px-4 py-2.5 rounded-full"} ${isActive ? "text-[var(--color-primary)] font-bold shadow-sm" : "text-[var(--color-text-muted)] font-medium hover:text-[var(--color-primary)]"}`}
                        >
                          {isActive ? (
                            <motion.div
                              layoutId="activeNavBackground"
                              className="absolute inset-0 bg-white/70 border border-white/90 z-[-1]"
                              style={{
                                borderRadius: isVertical ? "12px" : "9999px",
                              }}
                              initial={false}
                            ...
                            <div
                              className="absolute inset-0 bg-white/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-[-1]"
                              style={{
                                borderRadius: isVertical ? "12px" : "9999px",
                              }}
                            />
                          ...
                          <div
                            className={`relative flex items-center justify-center transition-all duration-300 ${isActive ? "scale-100 text-[var(--color-primary)]" : "scale-95 opacity-70 group-hover:opacity-100 group-hover:scale-100 group-hover:-translate-y-0.5"}`}
                          >
                            <svg
                          ...
                          {!isVertical && (
                            <span className="whitespace-nowrap text-[13px] lg:text-[14px] ml-1.5 transition-all duration-300 group-hover:translate-x-0.5">
                              {t(
                                `nav.${item.id}` as keyof typeof import("../translations").translations.en,
                              )}
                            </span>
                          )}
                        </a>
              ...
              <AnimatePresence>
                {!isBall && (
                  <motion.div
                    ...
                    exit={{ opacity: 0 }}
                    transition={{ duration: 0.2 }}
                    className={`flex items-center shrink-0 z-20 ${isVertical ? "flex-col w-full px-3 mt-auto gap-3" : "gap-3"}`}
                  >
                    <a
                      href="/#contact"
                    ...
                    >
                      ...
                      <div className="relative z-10 flex items-center gap-2">
                        <span
                          className={`whitespace-nowrap text-[13px] lg:text-[14px] font-bold tracking-wide ${isVertical ? "hidden" : "block"}`}
                        >
                          {t("nav.contact")}
                        </span>
                        <svg
                          className="w-[18px] h-[18px] shrink-0 transition-transform duration-300 group-hover:translate-x-1 group-hover:-translate-y-1"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
                    ...
                    {/* 🔥 2. زر تغيير اللغة المحدث كلياً: (الكوكب للعمودي / الشكل الاحترافي للأفقي) 🔥 */}
                    <button
                      type="button"
                      ...
                      className={
                        isVertical
                          ? "group relative flex items-center justify-center w-12 h-12 rounded-full border border-slate-200/80 bg-white/60 backdrop-blur-md shadow-sm transition-all duration-500 hover:border-[var(--color-primary)]/40 hover:bg-white hover:shadow-[0_0_20px_rgba(17,79,209,0.15)]"
                          : "group relative flex items-center justify-center gap-2 rounded-full border border-white/60 bg-white/40 backdrop-blur-md px-3 lg:px-4 py-2 text-xs lg:text-sm font-bold text-slate-700 shadow-sm transition-all duration-300 hover:bg-white/90 hover:border-[var(--color-primary)]/30 hover:text-[var(--color-primary)] hover:shadow-[0_8px_20px_rgba(17,79,209,0.12)] hover:-translate-y-0.5"
                      }
                    >
                      {isVertical ? (
                        <>
                        ...
                        <>
                          {/* زر الوضع الأفقي الفخم */}
                          <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[var(--color-primary)]/10 text-[11px] font-black text-[var(--color-primary)] transition-all duration-300 group-hover:bg-[var(--color-primary)] group-hover:text-white group-hover:shadow-[0_2px_8px_rgba(17,79,209,0.3)]">
                            {language === "en" ? "ع" : "EN"}
                          </span>
                          <span>
          ...
          {isMenuOpen && isMobile && (
            ...
            >
              ...
              >
                ...
                <span
                  className={`text-[15px] ${activeSection === "home" && pathname === "/" ? "font-bold" : "font-semibold"}`}
                >
                  {t("nav.home")}
                </span>
              </a>
              ...
              {NAV_ITEMS.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`text-[15px] ${isActive ? "font-bold" : "font-semibold"}`}
                    >
                      {t(
                        `nav.${item.id}` as keyof typeof import("../translations").translations.en,
                      )}
              ...
              >
                ...
                <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[var(--color-primary)]/10 text-xs font-black text-[var(--color-primary)]">
                  {language === "en" ? "ع" : "EN"}
                </span>
              </button>
