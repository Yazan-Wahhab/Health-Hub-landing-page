...
const PROJECTS_DATA: Project[] = [
  {
    ...
    description:
      "تطبيق النواة المركزية لإدارة السعة السريرية لأكثر من 500 سرير. تضمن العمل بناء مخططات متقدمة تقضي على تكرار البيانات وتسرع الاستعلامات الطبية.",
    stats: [
      ...
      <div className="w-full px-4 md:px-12 lg:px-16 relative z-10 max-w-[1600px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-5 lg:gap-6 relative z-20">
          {PROJECTS_DATA.map((project, idx) => {
            ...
            if (idx === 0)
              spanClass =
                "md:col-span-8 md:row-span-2 min-h-[380px] md:min-h-[450px] lg:min-h-[600px]";
            else if (idx === 1)
              spanClass =
                "md:col-span-4 md:row-span-1 min-h-[260px] md:min-h-[300px]";
            else if (idx === 2)
              spanClass =
                "md:col-span-4 md:row-span-1 min-h-[260px] md:min-h-[300px]";
            else if (idx === 3)
              spanClass =
                "md:col-span-6 md:row-span-1 min-h-[280px] md:min-h-[350px]";
            else if (idx === 4)
              spanClass =
                "md:col-span-6 md:row-span-1 min-h-[280px] md:min-h-[350px]";
    ...
    >
      <motion.div
        ...
        className={`group relative w-full h-full flex flex-col justify-end overflow-hidden rounded-2xl md:rounded-[2rem] bg-[#0f172a] ring-1 ring-inset ring-white/10 cursor-pointer transition-all duration-700 outline-none ${
          isPrimary
            ? "hover:ring-[var(--color-primary)] hover:shadow-[0_20px_50px_-15px_rgba(17,79,209,0.4)]"
            : "hover:ring-[var(--color-secondary)] hover:shadow-[0_20px_50px_-15px_rgba(13,148,104,0.4)]"
        }`}
      >
        <div className="absolute inset-0 w-full h-full overflow-hidden">
          ...
          />
          <div
            className={`absolute inset-0 mix-blend-multiply opacity-20 transition-opacity duration-700 group-hover:opacity-0 ${isPrimary ? "bg-[#0c3ba6]" : "bg-[#08704d]"}`}
          />
        ...
        <div className="relative p-5 md:p-8 lg:p-10 z-10 w-full h-full flex flex-col justify-end text-white">
          <div
            className={`absolute top-4 left-4 md:top-6 md:left-6 w-10 h-10 md:w-12 md:h-12 rounded-full backdrop-blur-lg border border-white/20 bg-white/10 flex items-center justify-center opacity-0 -translate-x-4 transition-all duration-500 ease-out group-hover:translate-x-0 group-hover:opacity-100 ${
              isPrimary
                ? "group-hover:bg-[#114fd1] group-hover:border-[#114fd1] shadow-[0_0_20px_rgba(17,79,209,0.5)]"
                : "group-hover:bg-[#0d9468] group-hover:border-[#0d9468] shadow-[0_0_20px_rgba(13,148,104,0.5)]"
            }`}
          >
            <svg
          ...
          <div>
            ...
            <h3
              className={`font-extrabold tracking-tight mb-2 text-white drop-shadow-md ${isHero ? "text-[26px] sm:text-3xl md:text-5xl leading-tight" : "text-xl sm:text-2xl md:text-3xl"}`}
            >
              {t(project.title)}
            </h3>
            ...
            <p className="font-medium text-[13px] md:text-sm lg:text-base flex items-center gap-2 drop-shadow-md text-gray-200">
              <svg
                className={`w-4 h-4 md:w-5 md:h-5 ${isPrimary ? "text-[#60a5fa]" : "text-[#34d399]"}`}
                fill="currentColor"
                viewBox="0 0 24 24"
              >
      ...
      >
        ...
        <div className="w-full md:w-[55%] p-5 sm:p-8 md:p-12 lg:p-16 overflow-y-auto flex flex-col justify-center relative order-2 md:order-1">
          ...
          <div className="relative z-10">
            ...
            <p
              className={`text-[11px] md:text-sm font-bold uppercase tracking-wider mb-5 md:mb-8 flex items-center gap-2 ${isPrimary ? "text-[#0c3ba6]" : "text-[#08704d]"}`}
            >
              <svg
            ...
            <div className="mt-auto">
              <h4 className="text-[10px] md:text-xs font-bold text-[#475569] uppercase tracking-widest mb-3 md:mb-5">
                {t("الأثر التشغيلي")}
              </h4>
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                {project.stats.map((stat, idx) => (
                  ...
                  >
                    <span
                      className={`block text-xl md:text-3xl font-black mb-1 ${isPrimary ? "text-[#114fd1]" : "text-[#0d9468]"}`}
                    >
                      {stat.value}
                    </span>
        ...
        <div className="w-full md:w-[45%] h-52 sm:h-64 md:h-auto relative overflow-hidden order-1 md:order-2 bg-[#0f172a] border-r border-white/40 flex-shrink-0">
          ...
          />
          <div
            className={`absolute inset-0 mix-blend-multiply opacity-20 ${isPrimary ? "bg-[#0c3ba6]" : "bg-[#08704d]"}`}
          />
          <div className="absolute inset-0 bg-gradient-to-t from-[#0f172a]/95 via-[#0f172a]/30 to-transparent pointer-events-none" />
        </div>
      </motion.div>
