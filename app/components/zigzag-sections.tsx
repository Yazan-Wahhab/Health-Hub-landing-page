...
import { motion, useScroll, AnimatePresence, useMotionValueEvent, useAnimation } from "framer-motion";
...
const FallingShapes = () => (
  // 🔥 PRO FIX: Added will-change-transform to CSS keyframe animations
  <div className="absolute inset-0 overflow-hidden pointer-events-none z-0 opacity-60">
    <div className="absolute w-2.5 h-2.5 rounded-full bg-[var(--color-secondary)] shape-fall delay-1 will-change-transform"></div>
    <div className="absolute w-2 h-2 bg-[#0EA5E9] rounded-sm shape-fall delay-2 rotate-45 will-change-transform"></div>
    <svg className="absolute w-4 h-4 text-[var(--color-secondary)] shape-fall delay-3 will-change-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M12 4v16m8-8H4" /></svg>
    <div className="absolute w-1.5 h-1.5 rounded-full bg-[var(--color-primary)] shape-fall delay-4 will-change-transform"></div>
    <svg className="absolute w-3 h-3 text-[#0EA5E9] shape-fall delay-5 will-change-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M12 4v16m8-8H4" /></svg>
    <div className="absolute w-3 h-3 rounded-full border border-[var(--color-secondary)] shape-fall delay-6 will-change-transform"></div>
  </div>
);
...
export default function ZigZagSections() {
  ...
  // Trigger opens early at 20% scroll, rendering direct to GPU
  useMotionValueEvent(scrollYProgress, "change", (latest) => {
    if (latest >= 0.01 && !hasTriggered.current) {
      hasTriggered.current = true;
      controls.start("visible");
    } else if (latest < 0.01 && hasTriggered.current) {
      hasTriggered.current = false;
      controls.start("hidden");
    }
  });

  // 🔥 PRO FIX: Reduced stiffness slightly so the animation doesn't spike CPU usage on entry
  ...
  const slideVariants = {
    enter: (direction: number) => ({ x: direction > 0 ? 150 : -150, opacity: 0, scale: 0.9, rotateY: direction > 0 ? 10 : -10, filter: "blur(8px)" }),
    center: { zIndex: 1, x: 0, opacity: 1, scale: 1, rotateY: 0, filter: "blur(0px)" },
    exit: (direction: number) => ({ zIndex: 0, x: direction < 0 ? 150 : -150, opacity: 0, scale: 0.9, rotateY: direction < 0 ? -10 : 10, filter: "blur(8px)" })
  };
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .parallax-layer { position: absolute; top: -50%; left: -50%; width: 200%; height: 200%; pointer-events: none; }
        .layer-1 { background-image: url("data:image/svg+xml,%3Csvg width='100' height='100' viewBox='0 0 100 100' xmlns='http://www.w3.org/2000/svg'%3E%3Cg opacity='0.2'%3E%3Ccircle cx='40' cy='50' r='10' fill='%230EA5E9'/%3E%3Ccircle cx='60' cy='50' r='10' fill='%2310B981'/%3E%3Cpath d='M40 50 L60 50' stroke='%23ffffff' stroke-width='2' opacity='0.5'/%3E%3C/g%3E%3C/svg%3E"); background-size: 100px 100px; animation: driftFast 15s linear infinite; }
        .layer-2 { background-image: url("data:image/svg+xml,%3Csvg width='220' height='220' viewBox='0 0 220 220' xmlns='http://www.w3.org/2000/svg'%3E%3Cg opacity='0.1'%3E%3Ccircle cx='90' cy='110' r='20' fill='%230EA5E9'/%3E%3Ccircle cx='130' cy='110' r='20' fill='%2310B981'/%3E%3Cpath d='M90 110 L130 110' stroke='%23ffffff' stroke-width='3' opacity='0.3'/%3E%3C/g%3E%3C/svg%3E"); background-size: 220px 220px; animation: driftSlow 30s linear infinite; }
      ...
      <div ref={mainContainerRef} className="relative w-full">
        ...
        <div className="block lg:hidden">
          <section className="relative h-[130vh] w-full bg-transparent z-20">
            <div className="sticky top-0 h-[100svh] w-full overflow-hidden flex items-center justify-center pointer-events-none">
              <div className="relative w-full h-[88svh] max-h-[800px] max-w-[420px] mx-auto pointer-events-auto">
                <motion.svg className="absolute inset-0 w-full h-full z-0 pointer-events-none will-change-transform transform-gpu [-webkit-backface-visibility:hidden]" preserveAspectRatio="none" variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { duration: 0.8 } } }} initial="hidden" animate={controls}>
                  <motion.line x1="25%" y1="18%" x2="25%" y2="82%" stroke="var(--color-primary)" strokeOpacity="0.2" strokeWidth="1.5" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1.5 } } }} />
                  <motion.line x1="25%" y1="18%" x2="25%" y2="82%" stroke="var(--color-primary)" strokeWidth="2" strokeDasharray="6 8" className="animate-data-flow opacity-60" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1.5 } } }} />
                  <motion.line x1="75%" y1="18%" x2="75%" y2="82%" stroke="var(--color-secondary)" strokeOpacity="0.2" strokeWidth="1.5" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1.5 } } }} />
                  <motion.line x1="75%" y1="18%" x2="75%" y2="82%" stroke="var(--color-secondary)" strokeWidth="2" strokeDasharray="6 8" className="animate-data-flow opacity-60" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1.5 } } }} />
                  <motion.line x1="25%" y1="50%" x2="38%" y2="50%" stroke="var(--color-primary)" strokeOpacity="0.2" strokeWidth="1.5" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1, delay: 0.2 } } }} />
                  <motion.line x1="25%" y1="50%" x2="38%" y2="50%" stroke="var(--color-primary)" strokeWidth="2" strokeDasharray="6 8" className="animate-data-flow opacity-60" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1, delay: 0.2 } } }} />
                  <motion.line x1="62%" y1="50%" x2="75%" y2="50%" stroke="var(--color-secondary)" strokeOpacity="0.2" strokeWidth="1.5" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1, delay: 0.2 } } }} />
                  <motion.line x1="62%" y1="50%" x2="75%" y2="50%" stroke="var(--color-secondary)" strokeWidth="2" strokeDasharray="6 8" className="animate-data-flow opacity-60" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1, delay: 0.2 } } }} />
                </motion.svg>

                {/* 🔥 PRO FIX: Added will-change-transform transform-gpu to all animating elements */}
                <motion.div className="absolute top-[10%] left-[2%] w-[46%] flex justify-center z-10 will-change-transform transform-gpu [-webkit-backface-visibility:hidden]" variants={{ hidden: { opacity: 0, x: 20, y: 20, scale: 0.9 }, visible: { opacity: 1, x: 0, y: 0, scale: 1, transition: { ...smoothSpring } } }} initial="hidden" animate={controls}>
                ...
                <motion.div className="absolute top-[6%] right-[2%] w-[46%] flex justify-center z-20 will-change-transform transform-gpu [-webkit-backface-visibility:hidden]" variants={{ hidden: { opacity: 0, x: -20, y: 20, scale: 0.9 }, visible: { opacity: 1, x: 0, y: 0, scale: 1, transition: { ...smoothSpring, delay: 0.1 } } }} initial="hidden" animate={controls}>
                ...
                <motion.div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[48%] max-w-[180px] z-30 will-change-transform transform-gpu [-webkit-backface-visibility:hidden]" variants={{ hidden: { opacity: 0, scale: 0.5 }, visible: { opacity: 1, scale: 1, transition: { ...smoothSpring, delay: 0.2 } } }} initial="hidden" animate={controls}>
                ...
                <motion.div className="absolute bottom-[8%] left-[2%] w-[46%] flex justify-center z-20 will-change-transform transform-gpu [-webkit-backface-visibility:hidden]" variants={{ hidden: { opacity: 0, x: 20, y: -20, scale: 0.9 }, visible: { opacity: 1, x: 0, y: 0, scale: 1, transition: { ...smoothSpring, delay: 0.3 } } }} initial="hidden" animate={controls}>
                ...
                <motion.div className="absolute bottom-[10%] right-[2%] w-[46%] flex justify-center z-10 will-change-transform transform-gpu [-webkit-backface-visibility:hidden]" variants={{ hidden: { opacity: 0, x: -20, y: -20, scale: 0.9 }, visible: { opacity: 1, x: 0, y: 0, scale: 1, transition: { ...smoothSpring, delay: 0.4 } } }} initial="hidden" animate={controls}>
                  <div className="relative w-full bg-[var(--color-secondary)]/15 backdrop-blur-xl border border-[var(--color-secondary)]/30 p-3.5 rounded-[1.2rem] shadow-[0_20px_50px_rgba(13,148,104,0.15)] text-center">
        ...
        <div className="hidden lg:block">
          <section className="relative h-[130vh] w-full bg-transparent z-10">
            <div className="sticky top-0 h-screen w-full overflow-hidden pointer-events-none">
              
              <motion.div className="absolute top-[4vh] left-1/2 -translate-x-1/2 z-50 bg-white/90 backdrop-blur-xl border border-[var(--color-border)] shadow-[0_15px_30px_rgba(0,0,0,0.06)] px-8 py-3.5 rounded-full text-center flex items-center gap-4 pointer-events-auto will-change-transform transform-gpu [-webkit-backface-visibility:hidden]" variants={{ hidden: { opacity: 0, y: -30, scale: 0.9 }, visible: { opacity: 1, y: 0, scale: 1, transition: { ...smoothSpring } } }} initial="hidden" animate={controls}>
              ...
              <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="absolute inset-0 w-full h-full z-10 pointer-events-none opacity-100 will-change-transform transform-gpu [-webkit-backface-visibility:hidden]">
                <motion.path d="M 50 10 C 50 16, 25 15, 25 22" fill="none" stroke="var(--color-primary)" strokeWidth="1.5" strokeOpacity="0.4" vectorEffect="non-scaling-stroke" strokeLinecap="round" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1.5 } } }} initial="hidden" animate={controls} />
                <motion.path d="M 50 10 C 50 16, 25 15, 25 22" fill="none" stroke="var(--color-primary)" strokeWidth="3" vectorEffect="non-scaling-stroke" strokeDasharray="6 8" className="animate-data-flow opacity-90" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1.5 } } }} initial="hidden" animate={controls} />
                <motion.path d="M 50 10 C 50 16, 75 15, 75 22" fill="none" stroke="var(--color-secondary)" strokeWidth="1.5" strokeOpacity="0.4" vectorEffect="non-scaling-stroke" strokeLinecap="round" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1.5 } } }} initial="hidden" animate={controls} />
                <motion.path d="M 50 10 C 50 16, 75 15, 75 22" fill="none" stroke="var(--color-secondary)" strokeWidth="3" vectorEffect="non-scaling-stroke" strokeDasharray="6 8" className="animate-data-flow opacity-90" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1.5 } } }} initial="hidden" animate={controls} />
                <motion.line x1="25" y1="22" x2="75" y2="22" stroke="var(--color-primary)" strokeWidth="1.5" strokeOpacity="0.3" vectorEffect="non-scaling-stroke" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1, delay: 0.2 } } }} initial="hidden" animate={controls} />
                <motion.line x1="25" y1="22" x2="75" y2="22" stroke="var(--color-primary)" strokeWidth="3" vectorEffect="non-scaling-stroke" strokeDasharray="6 8" className="animate-data-flow opacity-90" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1, delay: 0.2 } } }} initial="hidden" animate={controls} />
                <motion.line x1="25" y1="40" x2="25" y2="56" stroke="var(--color-primary)" strokeWidth="1.5" strokeOpacity="0.3" vectorEffect="non-scaling-stroke" strokeLinecap="round" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1.5, delay: 0.3 } } }} initial="hidden" animate={controls} />
                <motion.line x1="25" y1="40" x2="25" y2="56" stroke="var(--color-primary)" strokeWidth="3" vectorEffect="non-scaling-stroke" strokeDasharray="6 8" className="animate-data-flow opacity-90" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1.5, delay: 0.3 } } }} initial="hidden" animate={controls} />
                <motion.line x1="75" y1="48" x2="75" y2="56" stroke="var(--color-secondary)" strokeWidth="1.5" strokeOpacity="0.3" vectorEffect="non-scaling-stroke" strokeLinecap="round" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1.5, delay: 0.3 } } }} initial="hidden" animate={controls} />
                <motion.line x1="75" y1="48" x2="75" y2="56" stroke="var(--color-secondary)" strokeWidth="3" vectorEffect="non-scaling-stroke" strokeDasharray="6 8" className="animate-data-flow opacity-90" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1.5, delay: 0.3 } } }} initial="hidden" animate={controls} />
                <motion.path d="M 25 72 C 25 78, 42 76, 50 76" fill="none" stroke="var(--color-primary)" strokeWidth="1.5" strokeOpacity="0.3" vectorEffect="non-scaling-stroke" strokeLinecap="round" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1, delay: 0.4 } } }} initial="hidden" animate={controls} />
                <motion.path d="M 25 72 C 25 78, 42 76, 50 76" fill="none" stroke="var(--color-primary)" strokeWidth="3" vectorEffect="non-scaling-stroke" strokeDasharray="6 8" className="animate-data-flow opacity-90" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1, delay: 0.4 } } }} initial="hidden" animate={controls} />
                <motion.path d="M 75 72 C 75 78, 58 76, 50 76" fill="none" stroke="var(--color-secondary)" strokeWidth="1.5" strokeOpacity="0.3" vectorEffect="non-scaling-stroke" strokeLinecap="round" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1, delay: 0.4 } } }} initial="hidden" animate={controls} />
                <motion.path d="M 75 72 C 75 78, 58 76, 50 76" fill="none" stroke="var(--color-secondary)" strokeWidth="3" vectorEffect="non-scaling-stroke" strokeDasharray="6 8" className="animate-data-flow opacity-90" variants={{ hidden: { pathLength: 0 }, visible: { pathLength: 1, transition: { duration: 1, delay: 0.4 } } }} initial="hidden" animate={controls} />
              </svg>

              <motion.div className="absolute top-[22vh] left-1/2 z-20 flex items-center justify-center pointer-events-none will-change-transform transform-gpu [-webkit-backface-visibility:hidden]" variants={{ hidden: { x: "-50%", y: "-50%", opacity: 0, scale: 0.5 }, visible: { opacity: 1, scale: 1, transition: { ...smoothSpring, delay: 0.2 } } }} initial="hidden" animate={controls}>
              ...
              <motion.div className="absolute top-[10vh] left-1/2 z-30 flex flex-col items-center w-[40vw] max-w-[30rem] pointer-events-auto will-change-transform transform-gpu [-webkit-backface-visibility:hidden]" variants={{ hidden: { x: "calc(-50% - 20vw)", y: 20, opacity: 0, scale: 0.95 }, visible: { x: "calc(-50% - 25vw)", y: 0, opacity: 1, scale: 1, transition: { ...smoothSpring, delay: 0.1 } } }} initial="hidden" animate={controls}>
                <div onClick={() => setActiveGallery('web')} className="relative w-full flex flex-col items-center group cursor-pointer transition-transform duration-300 hover:-translate-y-2">
                  ...
                  <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 flex items-center justify-center w-3.5 h-3.5 rounded-full bg-white border-2 border-[var(--color-primary)] shadow-[0_0_15px_rgba(17,79,209,0.4)] z-10 animate-pulse"></div>
                </div>
              </motion.div>

              <motion.div className="absolute top-[10vh] left-1/2 z-30 flex flex-col items-center w-[18vw] max-w-[12rem] pointer-events-auto will-change-transform transform-gpu [-webkit-backface-visibility:hidden]" variants={{ hidden: { x: "calc(-50% + 20vw)", y: 20, opacity: 0, scale: 0.95 }, visible: { x: "calc(-50% + 25vw)", y: 0, opacity: 1, scale: 1, transition: { ...smoothSpring, delay: 0.2 } } }} initial="hidden" animate={controls}>
                <div onClick={() => setActiveGallery('mobile')} className="relative w-full aspect-[9/19.5] bg-[#050505] rounded-[2rem] p-1.5 border-[4px] border-[#222] shadow-[0_25px_50px_rgba(13,148,104,0.15)] overflow-hidden group cursor-pointer transition-transform duration-300 hover:-translate-y-2">
                  ...
                  {/* Perfectly Centered Dot */}
                  <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 flex items-center justify-center w-3.5 h-3.5 rounded-full bg-white border-2 border-[var(--color-secondary)] shadow-[0_0_15px_rgba(13,148,104,0.4)] z-50 animate-pulse"></div>
                </div>
              </motion.div>

              <motion.div className="absolute top-[56vh] left-1/2 z-30 w-[38vw] max-w-[26rem] flex flex-col items-center group transition-colors duration-500 will-change-transform transform-gpu [-webkit-backface-visibility:hidden]" variants={{ hidden: { x: "calc(-50% - 20vw)", y: -20, opacity: 0, scale: 0.95 }, visible: { x: "calc(-50% - 25vw)", y: 0, opacity: 1, scale: 1, transition: { ...smoothSpring, delay: 0.3 } } }} initial="hidden" animate={controls}>
                ...
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 flex items-center justify-center w-4 h-4 rounded-full bg-[#f0f6ff] border-2 border-[var(--color-secondary)] z-20"><div className="w-1.5 h-1.5 rounded-full bg-[var(--color-secondary)]"></div></div>
                <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 flex items-center justify-center w-4 h-4 rounded-full bg-[#f0f6ff] border-2 border-[var(--color-secondary)] z-20"><div className="w-1.5 h-1.5 rounded-full bg-[var(--color-secondary)]"></div></div>
              </motion.div>

              <motion.div className="absolute top-[56vh] left-1/2 z-30 w-[38vw] max-w-[26rem] flex flex-col items-center group transition-colors duration-500 will-change-transform transform-gpu [-webkit-backface-visibility:hidden]" variants={{ hidden: { x: "calc(-50% + 20vw)", y: -20, opacity: 0, scale: 0.95 }, visible: { x: "calc(-50% + 25vw)", y: 0, opacity: 1, scale: 1, transition: { ...smoothSpring, delay: 0.4 } } }} initial="hidden" animate={controls}>
                <div className="relative w-full bg-[var(--color-secondary)]/15 backdrop-blur-xl border border-[var(--color-secondary)]/30 p-6 rounded-[2rem] shadow-[0_20px_50px_rgba(13,148,104,0.15)] text-center group-hover:bg-[var(--color-secondary)]/25 transition-colors">
                  <div className="absolute inset-0 rounded-[2rem] overflow-hidden pointer-events-none z-0"><FallingShapes /></div>
              ...
              <motion.div className="absolute bottom-[3vh] left-1/2 -translate-x-1/2 z-40 w-[85vw] max-w-[42rem] flex flex-col items-center will-change-transform transform-gpu [-webkit-backface-visibility:hidden]" variants={{ hidden: { opacity: 0, y: 30, scale: 0.95 }, visible: { opacity: 1, y: 0, scale: 1, transition: { ...smoothSpring, delay: 0.5 } } }} initial="hidden" animate={controls}>
                ...
                <div className="absolute -top-6 left-1/2 -translate-x-1/2 flex items-center justify-center w-12 h-12 rounded-xl bg-[#064e3b] border-2 border-[var(--color-secondary)] shadow-[0_10px_20px_rgba(13,148,104,0.4)] z-50"><svg className="w-6 h-6 text-[#10B981]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg></div>
              </motion.div>
            </div>
          </section>
        </div>
