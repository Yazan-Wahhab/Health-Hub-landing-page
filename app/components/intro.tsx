...
import Logo3D from "./logo"; // تأكد من مطابقة مسار الاستيراد للملف الأول
...
export default function Intro({ onComplete }: IntroProps) {
  ...
  return (
    <AnimatePresence>
      {phase !== "done" ? (
        <motion.div
          // 💡 إجبار الـ Wrapper الأساسي للانترو على البقاء LTR لتجنب أي عشوائية في الاتجاهات بسبب اللغة
          dir="ltr"
          className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden bg-gradient-to-br from-[#0a1b3f] via-[#0d2a73] to-[#0a1b3f] will-change-transform transform-gpu"
          initial={{ opacity: 0 }}
          animate={
            phase === "exit"
              ? {
        ...
        >
          ...
          <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
            <motion.div
              ...
              className={`absolute top-0 left-0 h-[80vh] w-[80vh] rounded-full bg-[#114FD1] opacity-30 ${isMobile ? "blur-[80px]" : "blur-[160px]"}`}
            />
            <motion.div
              ...
              className={`absolute bottom-0 right-0 h-[70vh] w-[70vh] rounded-full bg-[#10B981] opacity-20 ${isMobile ? "blur-[80px]" : "blur-[150px]"}`}
            />
          </div>
          ...
          <div className="relative z-20 flex flex-col items-center justify-center text-center pointer-events-none mt-24 px-4 w-full">
            <AnimatePresence>
              {textVisible && (
                <motion.div
                  key="text-container"
                  className="flex flex-col items-center gap-4 sm:gap-6 w-full will-change-transform transform-gpu"
                  initial="hidden"
                  animate="show"
                  exit="exit"
                  variants={{
                ...
                >
                  ...
                  <motion.p
                    variants={{
                      ...
                      show: { opacity: 1, y: 0, filter: "none" },
                      exit: {
                        opacity: 0,
                        y: 10,
                        filter: isMobile ? "none" : "blur(5px)",
